...
// ## Constructor
// 
// * `host` (`String`) -- The hostname of the server. Required.
// * `callback` (`Function`) -- This will run after the server has successfully connected. Optional.
function Minecraft (host, callback) {
    var self = this;

    this.connection = net.connect({port: 4711, host: host}, function () {
        // When a new `Minecraft` is created, it connects to the host given at port 4711, and preforms the callback, if it exists, once connected.
        console.log('Connected to server!');
        self.chat('Now being commanded from ' + os.hostname());
        if (callback) {
            callback();
        };
    });
...
Minecraft.prototype.destroy = function () {
    this.chat(os.hostname() + ' has stopped commanding.');
    this.connection.destroy();
    process.exit();
};

// ## Commands
// `var client = new Minecraft('localhost');`
